...
int vm_step (struct _vm * vm)
{
    ...
    case OP_JNE : if (vm->flags != 0) vm->regs[REG_IP] += ntohs(ins->lval); break;
    case OP_JL  : if (vm->flags <  0) vm->regs[REG_IP] += ntohs(ins->lval); break;
    case OP_JLE : if (vm->flags <= 0) vm->regs[REG_IP] += ntohs(ins->lval); break;
    case OP_JG  : if (vm->flags >  0) vm->regs[REG_IP] += ntohs(ins->lval); break;
    case OP_JGE : if (vm->flags >= 0) vm->regs[REG_IP] += ntohs(ins->lval); break;
